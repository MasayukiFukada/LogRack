defmodule LogLacWeb.SensorController do
  ...
  def index(conn, _params) do
    sensors = Member.list_sensors()
    render(conn, "index.html", sensors: sensors)
  end

  def new(conn, _params) do
    changeset = Member.change_sensor(%Sensor{})
    render(conn, "new.html", changeset: changeset)
  end

  def create(conn, %{"sensor" => sensor_params}) do
    case Member.create_sensor(sensor_params) do
      {:ok, sensor} ->
        conn
  ...
  def update(conn, %{"id" => id, "sensor" => sensor_params}) do
    ...
    case Member.update_sensor(sensor, sensor_params) do
      {:ok, sensor} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    sensor = Member.get_sensor!(id)
    {:ok, _sensor} = Member.delete_sensor(sensor)
